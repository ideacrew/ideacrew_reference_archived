...
module Profiles
  class Profile
    ...
    embeds_many :office_locations,
                class_name:"Locations::OfficeLocation", cascade_callbacks: true

    # embeds_one  :inbox, as: :recipient, cascade_callbacks: true,
    #   class_name:"Inboxes::Inbox"
    ...
    class << self
      def find(id)
        ...
        organization = Organizations::Organization.where("profiles._id" => BSON::ObjectId.from_string(id)).first
        organization.profiles.detect { |profile| profile.id.to_s == id.to_s } if organization.present?
      end

      def by_hbx_id(hbx_id)
        organization = Organizations::Organization.where(hbx_id: hbx_id, profiles: {"$exists" => true})
